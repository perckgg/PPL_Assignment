...
from main.zcode.codegen.CodeGenError import *
...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is NumberType:
            ...
            arr = ""
            #print("inType",inType)
            for x in range(len(inType.size)):
                arr += "["
            return arr + self.getJVMType(inType.eleType) if arr != "" else "[" + self.getJVMType(inType.eleType)
        elif typeIn is cgen.ZCodeType:
            return "(" + "".join(list(map(lambda x: self.getJVMType(x), inType.starttype))) + ")" + self.getJVMType(inType.rettype)
        # elif typeIn is cgen.ZCodeType:
        #     return "(" + "".join(list(map(lambda x: self.getJVMType(x) if type(x) is not tuple else self.getJVMType(x[0]), inType.starttype))) + ")" + self.getJVMType(inType.rettype)
        elif typeIn is str:
            return inType
    ...
    def getFullType(self,inType):
        ...
        if typeIn is NumberType:
            ...
            for x in range(len(inType.size)):
                arr += "["
            return arr + self.getFullType(inType.eleType) if arr != "" else "[" + self.getFullType(inType.eleType)
        elif typeIn is cgen.ZCodeType:
            return "(".join(list(map(lambda x: self.getFullType(x), inType.starttype))) + ")" + self.getFullType(inType.rettype)
        elif typeIn is str:
            return inType
        elif typeIn is cgen.ClassType:
            return "L" + inType.cname + ";"
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):### need to change
        # in_: String
        # typ: Type
        # frame: Frame
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitIREADVAR(self, name, inType, index, frame):
        frame.push()
        if type(inType) is BoolType:
            return self.jvm.emitILOAD(index)
        elif type(inType) is StringType or type(inType) is ArrayType:
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isFinal, value = None):
        ...
        return self.jvm.emitSTATICFIELD(lexeme, self.getJVMType(in_), False) #change false to False
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESTATIC(self, lexeme, in_, frame):
        ...
        typ = in_
        list(map(lambda x: frame.pop(), typ.starttype))
    ...
    *   @param in the type of the operands.
    ...
    def emitIADDOP(self, lexeme, in_, frame):
        # lexeme: String
        # in_: Type
        # frame: Frame
        # ..., value1, value2 -> ..., result
    ...
    *   @param in the type of the operands.
    ...
    def emitIMULOP(self, lexeme, in_, frame):
        # lexeme: String
        # in_: Type
        # frame: Frame
        # ..., value1, value2 -> ..., result
    ...
    *   generate iand
    '''
    def emitIINC (self, index, step):
        return self.jvm.emitIINC(index, step)
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def emitMULTIANEWARRAY(self, in_, frame):
        ...
        if type(in_) is ArrayType:
            dimens = len(in_.dimensions)
            val = val = self.getJVMType(in_)
            return self.jvm.emitMULTIANEWARRAY(val, dimensions=dimens)
    ...
    *   @param in the code to be printed out
    ...
    def emitARRAYLITERAL(self, array_type, frame):
        code = ""
        for dim in array_type.size:
            code += self.emitPUSHICONST(dim, frame)
        typ = self.getJVMType(array_type.eleType) if isinstance(array_type.eleType, ArrayType) else self.getFullType(array_type.eleType)
        code += self.jvm.emitANEWARRAY(typ) if (type(array_type.eleType) in [ArrayType, StringType]) else self.jvm.emitNEWARRAY(typ)
        return code
    def emitDUP(self, frame):
        # frame: Frame
